...
const doctype = computed(() => {
  const m = props.value.match(/^\s*(<!DOCTYPE[^>]*>)/i)
  return m ? m[1] : ''
})
...
const parsed = computed(() => {
  try {
    ...
    const hasHtmlRoot = /<html\b/i.test(props.value)
    const nodes: Node[] = []
    if (hasHtmlRoot && doc.documentElement) {
      nodes.push(doc.documentElement)
    } else {
      // Fragment input - show body children directly so users don't see
      // the synthetic <html><head><body> wrapper inserted by the parser.
      const body = doc.body
      if (body) {
        body.childNodes.forEach((n) => {
          if (n.nodeType === 1 || n.nodeType === 8) nodes.push(n)
        })
      }
    }
    ...
    <div v-else class="flex-1 flex overflow-auto">
      ...
      <div ref="treeEl" class="flex-1 font-mono text-xs leading-relaxed py-4 -ml-12" :class="softWrap ? 'break-all' : 'whitespace-nowrap'">
        <div class="json-tree-content" :key="treeKey">
          <span v-if="doctype" class="json-line relative">
            <span class="json-ln">
              <span class="json-ln-num"></span>
              <span class="json-ln-toggle" aria-hidden="true" />
            </span>
            <span class="text-purple-600 dark:text-purple-400">{{ doctype }}</span>
          </span>
          <XmlNode
